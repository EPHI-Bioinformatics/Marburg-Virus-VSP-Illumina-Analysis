...
#   Nextclade analysis, and MultiQC reporting.
###############################################################################
...
mkdir -p "$FASTP_DIR" "$HOSTREM_DIR" "$NONHOST_DIR" "$MAP_DIR" "$MAPPING_QC_DIR" \
         "$VAR_DIR" "$CONS_DIR" "$COV_DIR" "$NEXTCLADE_DIR" "$MULTIQC_DIR"
...
# STEP 8: Nextclade analysis
print_step "STEP 8: Nextclade analysis"
bash "$SCRIPTS_DIR/nextclade_batch.sh"
echo -e "${GREEN}✅ Nextclade analysis completed.${NC}\n"

# STEP 9: MultiQC (Fastp + Qualimap + Coverage + Nextclade)
print_step "STEP 9: MultiQC reporting"
