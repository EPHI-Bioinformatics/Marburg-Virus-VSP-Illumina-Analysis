...
echo "Starting mapping with Minimap2." | tee -a "$LOG_FILE"
...
[[ -z "$MINIMAP2_EXEC" || -z "$SAMTOOLS_EXEC" ]] && echo "Missing tools." && exit 1
...
for fq1 in "$NONHOST_DIR"/*_1.nonhost.fastq.gz; do
    ...
    (
        "$MINIMAP2_EXEC" -ax sr -t "$THREADS" "$REFERENCE" "$fq1" "$fq2" | \
    ...
    MAPPED=$("$SAMTOOLS_EXEC" view -F 4 -c "$outbam" || echo 0)
